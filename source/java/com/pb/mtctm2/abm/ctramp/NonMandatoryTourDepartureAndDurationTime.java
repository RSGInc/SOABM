        ...
        for ( int p=1; p < persons.length; p++ ) {
            ...
            for ( int i=0; i < tourPurposeIndexOrder.length; i++ ) {
                ...
                for ( Tour t : purposeTourLists[tourPurposeIndex] ) {
                    ...
                    try {
                        ...
                        if ( alternativeAvailable < 0 ) {
                            ...
                            // schedule the chosen alternative
                           	//skip re-scheduling partially joint tours.
                        	if(t.getEscortTypeOutbound()==0 && t.getEscortTypeInbound()==0){
                        		...
                        		t.setTourDepartPeriod(escortTourDepartPeriod);
                        		t.setTourArrivePeriod(escortTourArrivePeriod);
                        	}
                            ...
                            if ( runModeChoice ) {
                                ...
                                int chosenMode = mcModel.getModeChoice(mcDmuObject, t.getTourPrimaryPurpose());
                               	...
                               	//skip set mode for partially joint tours.
                            	if(t.getEscortTypeOutbound()==0 && t.getEscortTypeInbound()==0)
                            ...
                            if ( runModeChoice ) {
                                ...
                                int chosenMode = mcModel.getModeChoice(mcDmuObject, t.getTourPurpose());
                              	...
                              	//skip mode for partially joint tours.
                            	if(t.getEscortTypeOutbound()==0 && t.getEscortTypeInbound()==0)
                            		t.setTourModeChoice(chosenMode);
                    ...
                    tourPurpNum++;
                   	if((t.getEscortTypeOutbound()>0 || t.getEscortTypeInbound()>0) ){
                		if(t.getTourDepartPeriod()<=0 || t.getTourArrivePeriod()<=0){
                			logger.fatal("Error: Escort tour arrival period and/or departure period is <=0 at nonmandatory tour departure and duration model");
                			hh.logEntireHouseholdObject("Debug for hh "+hh.getHhId()+" escort error", logger);
                			throw new RuntimeException();
                		}
