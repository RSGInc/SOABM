...
public class HouseholdDataWriter
{
	...
	public static final String  PROPERTIES_RUN_UNI_PARK_MODEL   = "UniversityParkingLotChoiceModel.RunModel";
    ...
    private ParkingLotChoiceModel parkingLotChoiceModel;
    private boolean runUniversityParkingChoiceModel;
    private ModelStructure      modelStructure;
    private int                 iteration;
    private float[][][][] parkingDemand;
    private float occupancyThreePlus = 3.3f;
    private float sampleRate=1.0f;
    private PrintWriter parkingWriter;
    
    public HouseholdDataWriter(HashMap<String,String> rbMap, ModelStructure modelStructure,
            int iteration, float sampleRate)
    {
        ...
        //setup the parking lot choice model
        String runUniversityParkingModelString = rbMap.get(PROPERTIES_RUN_UNI_PARK_MODEL);
        if(runUniversityParkingModelString != null)
        {
        	if (runUniversityParkingModelString.equalsIgnoreCase("true")) runUniversityParkingChoiceModel = true;
        }
       
        if(runUniversityParkingChoiceModel){
        	ArrayList<Integer> universityZones = mgraManager.getMajorUniversityMgras();
        	parkingLotChoiceModel = new ParkingLotChoiceModel(rbMap,modelStructure,iteration,universityZones,sampleRate);
        	parkingLotChoiceModel.setup();
        }
    ...
    private void writeData(HouseholdDataManagerIf householdDataManager, DataWriter writer)
    {
        ...
        if(runUniversityParkingChoiceModel)
        	ParkingLotDataManager.resetAvailableSpaces();
               
        //first calculate parking location choice
        Household[] householdArray = householdDataManager.getHhArray();
        
        if(runUniversityParkingChoiceModel){
        	if(parkingLotChoiceModel.isSimulationModel()){
        		parkingLotChoiceModel.simulateParkingChoice(householdArray);
           	}
        	parkingLotChoiceModel.generateParkingTrips(householdArray);
    	}
        ...
        try
        {
            ...
            for (int[] startEndIndices : startEndTaskIndicesList)
            {
                ...
                householdArray = householdDataManager.getHhArray(startIndex, endIndex);
...
//                    if (size > maxSize) maxSize = size;
            ...
            if(runUniversityParkingChoiceModel)
            	writeParkingDemand(parkingWriter);
...
//            logger.info("max size for all Household objects after writing output files is " + maxSize + " bytes.");
        ...
        if(runUniversityParkingChoiceModel)        
        	parkingLotChoiceModel.writeUnmetDemand();
    ...
    private List<String> formIndivTripDataEntry(Stop s)
    {
        ...
        data.add(string(s.isUniversityParkingLot() ? 1 : 0));
  ...
  /*      //outbound chauffeured school tour no stops; origin stop type and escortee is zero, dest stop type is dropoff, escortee is pnum.
        ...
        */
        ...
        //not sure if this should be here; looks like it should be a set...
        data.add(string(t.getTourModeChoice()));
            ...
            implements DataWriter
    {
        ...
        public FileDataWriter()
        {
            ...
            String parkingFile = formFileName(rbMap.get(PROPERTIES_OUTPUT_PARKING_FILE), iteration);
            ...
            try
            {
                ...
                if(runUniversityParkingChoiceModel)
                	parkingWriter =  new PrintWriter(new File(baseDir + parkingFile));
            } catch (IOException e)
            {
                throw new RuntimeException(e);
            }
            ...
            if(runUniversityParkingChoiceModel)
            	writeParkingFileHeader(parkingWriter);	
    ...
    private double calculateTimesForAllMgras(int oMgra, int dMgra)
    {
        ...
        // sov time in results[0] and distance in results[1]
        double[] results = autoSkimUEC.solve(iv, dmu, null);
	...
	/**
	 * Write the header of the parking lot demand file
	 * @param writer
	 */
	private void writeParkingFileHeader(PrintWriter writer){

		String header = "MAZ,spaceType";
    	int periods  = ModelStructure.MAX_TOD_INTERVAL;

		for(int i = 0; i< ParkingLotChoiceModel.parkingSegmentNames.length;++i){
			
			String segmentName = ParkingLotChoiceModel.parkingSegmentNames[i];
			
			for(int j = 0; j < periods; ++j)
				header += ","+segmentName+"Demand_"+(j+1);
		...
		int maxSpaceType = Byte.MAX_VALUE;
		
		parkingDemand = new float[maxMgra][maxSpaceType][ParkingLotChoiceModel.parkingSegmentNames.length][periods];
		
	}
	
	/**
	 * Count the demand for the vehicle at the lot, according to the tour departure and arrival times,
	 * the market segment, the mode (for vehicle occupancy) and the expansion factor.
	 * 
	 * @param household  Needed for expansion factor.
	 * @param person     Needed for market segment.
	 * @param tour       Needed for mode and departure/arrival times.
	 */
	private void countParkingDemand(Household household, Person person, Tour tour){
		
		int parkingMgra = tour.getTourParkMgra();
		byte spaceType = tour.getTourParkSpaceType();
		...
		int marketSegment = 0;
		if(person==null) //joint tour, assume visitor
			marketSegment = ParkingLotChoiceModel.VISITOR_SEGMENT;
		else if(person.isMajorUniversityStudent())
			marketSegment = ParkingLotChoiceModel.STUDENT_SEGMENT;
		else if(person.isMajorUniversityWorker())
			marketSegment = ParkingLotChoiceModel.FACULTYSTAFF_SEGMENT;
		else
			marketSegment = ParkingLotChoiceModel.VISITOR_SEGMENT;
		

		//iterate through the time periods that the vehicle is at the lot, according to tour departure\arrival times
		for(int i = departPeriod; i <= arrivePeriod; ++i)
			parkingDemand[parkingMgra][spaceType][marketSegment][i-1] += demand;
			
	}
	
	/**
	 * Write the parking demand to the file.
	 * @param writer
	 */
	private void writeParkingDemand(PrintWriter writer){
		
		ArrayList<Integer> mazs = mgraManager.getMgras();
		
		// mazs
		for(int i = 0; i < parkingDemand.length;++i){
			
			//space types
			for(int j = 0; j < parkingDemand[i].length;++j){
				
				float lotDemand = 0f;
				
				//check if there is any demand for any market segment in any period for this lot
				
				//market segments
				for(int k = 0; k < parkingDemand[i][j].length;++k){
					
					//periods
					for(int l = 0; l < parkingDemand[i][j][k].length;++l){
						lotDemand += parkingDemand[i][j][k][l];
					}
				}
				
				//there is demand, so write the result out
				if(lotDemand > 0){
					String record = "" + mazs.get(i) +","+ j; //TAZ,spaceType
					
					for(int k = 0; k < parkingDemand[i][j].length;++k)
						for(int l = 0; l < parkingDemand[i][j][k].length;++l)
							record += ","+parkingDemand[i][j][k][l];
