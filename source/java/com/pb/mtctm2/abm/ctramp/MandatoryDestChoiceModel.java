...
{                                                                                                                                                                                                 
    ...
    {                                                                                                                                                                                             
        ...
        {                                                                                                                                                                                         
            ...
            //skip choice if person is major university student
            if(p.isMajorUniversityStudent()){
                // get the mode choice logsum for the destination choice sample alternative
                int segmentIndex = segmentNameIndexMap.get(  BuildAccessibilities.SCHOOL_DC_SIZE_SEGMENT_NAME_LIST[BuildAccessibilities.UNIV_TYPICAL_SEGMENT_GROUP_INDEX] );                      
                int segmentType = BuildAccessibilities.UNIV_TYPICAL_ALT_INDEX;                                                                                                                    

                float logsum = (float) getModeChoiceLogsum( hh, p, p.getCampusMaz(), segmentType);                                                                                                 
                mcModel.getAnmSkimCalculator().getAmPkSkimDistancesFromMgra( homeMgra, mgraDistanceArray );                                                                                       
                float distance = (float) mgraDistanceArray[p.getCampusMaz()];                                                                                                                
                p.setSchoolLocDistance(distance);
                p.setSchoolLocLogsum(logsum);
