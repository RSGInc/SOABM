        ...
        implements Serializable
{
    ...
    */
    public double getNMotorExpUtility(int iMgra, int jMgra, int period)
    {
        ...
        int iTaz = mgraManager.getTaz(iMgra);
        int jTaz = mgraManager.getTaz(jMgra);
        double nMotorExpUtility=0;
        if(nMotorExpUtilities[period][iTaz]==null){
            double nmtUtility = nMotorUEC[OFFPEAK_PERIOD_INDEX].calculateUtilitiesForTazPair(iTaz, jTaz);
            // exponentiate the SOV utility
            if (nmtUtility > -500)
	           nMotorExpUtility = Math.exp(nmtUtility);
        }else{
           nMotorExpUtility = nMotorExpUtilities[period][iTaz][jTaz];
        }
        return nMotorExpUtility;
    }
    ...
    {
    	double sovExpUtility=0;
    	if(sovExpUtilities[period][pTaz]==null){
    		double sovUtility = sovUEC[period].calculateUtilitiesForTazPair(pTaz, aTaz, avgTazHourlyParkingCost[aTaz]);
    		// exponentiate the SOV utility
    		if (sovUtility > -500)
    			sovExpUtility = Math.exp(sovUtility);
    	}else{
    		sovExpUtility = sovExpUtilities[period][pTaz][aTaz];
    	}
    	return sovExpUtility;
    }
