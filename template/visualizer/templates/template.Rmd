...
if(SCATTER_JPEG=="Yes"){
  ...
  p2 <- ggplot(counts_df, aes(x=x, y=y)) + 
    ...
    geom_text(x = lab_pos_x, y = lab_pos_y,label = lm_eqn(counts_df),  parse = TRUE, color = "#0072B2", size = 6) + 
...
```{r count_vol1}
if(IS_BASE_SURVEY=="Yes"){
  # both count and volume summaries for count_locations are in base summary folder
...
p2 <- ggplot(counts_df, aes(x=x, y=y)) + 
  ...
  geom_smooth(method=lm, formula = y ~ x - 1, se=FALSE, color = "#0072B2") + 
  geom_abline(intercept = 0, slope = 1, linetype = 2) + 
  geom_text(x = x_lab_pos, y = y_lab_pos,label = lm_eqn(counts_df),  parse = FALSE, color = "#0072B2", size = 6) + 
