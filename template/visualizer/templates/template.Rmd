...
Please set the browser zoom to 90% for optimal sizing of charts. 
...
if(IS_BASE_SURVEY=="Yes"){
  valueBox(prettyNum(round(build_df$value[build_df$name=="total_jobs"]), big.mark = ","), "Employment", icon = "fas fa-industry")
}else{
  valueBox(prettyNum(round(base_df$value[base_df$name=="total_jobs"]), big.mark = ","), "Employment", icon = "fas fa-industry")
}  
...
### Base VMT
```{r VMT1_ValueBox}
valueBox(prettyNum(round(base_df$value[base_df$name=="total_vmt"]/BASE_SAMPLE_RATE), big.mark = ","), "Total Resident VMT", icon = "ion-android-car")
```
...
### Build VMT
```{r VMT2_ValueBox}
valueBox(prettyNum(round(build_df$value[build_df$name=="total_vmt"]/BUILD_SAMPLE_RATE), big.mark = ","), "Total Resident VMT", icon = "ion-android-car")
```


Chart Column 1 {data-width=275}
--------------------------------------------
### Person Type Distribution {data-height=100}
```{r Chart_Person_Type}
...
### Household Size Distribution {data-height=100}
```{r Chart_HHSize}
...
### Average HH Size{data-height=75}
```{r avghhsize1}

#cat("Average HH Size: ", BASE_SCENARIO_NAME, " vs. ", BUILD_SCENARIO_NAME)

base_df <- base_data[[which(base_csv_names=="totals")]]
build_df <- build_data[[which(build_csv_names=="totals")]]
rate1 <- base_df$value[base_df$name=="total_population"]/base_df$value[base_df$name=="total_households"]
#gauge1 <- gauge(round(rate1, 2), min = 0, max = 5, gaugeSectors(danger =  c(0,5), colors = c("Green", "Green", "Green")))

rate2 <- build_df$value[build_df$name=="total_population"]/build_df$value[build_df$name=="total_households"]
#gauge2 <- gauge(round(rate2, 2), min = 0, max = 5, gaugeSectors(danger =  c(0,5), colors = c("Green", "Green", "Green")))
#
#bscols(widths = c(6,6),
#  gauge1,
#  gauge2
...
y <- c(BASE_SCENARIO_NAME,BUILD_SCENARIO_NAME)
x <- c(rate1,rate2)
df <- data.frame(y,x)

p1 <- plot_ly(x = ~x, y = ~reorder(y, x), name = 'Avg HH Size',
  ...
  add_annotations(xref = 'x1', yref = 'y',
                  x = x + 0.1,  y = y,
                  text = round(x, 2),
                  font = list(family = 'Arial', size = 18, color = 'rgb(50, 171, 96)'),
                  showarrow = FALSE)
...
```{r JobRate3_Gauge}
if(IS_BASE_SURVEY=="Yes"){
  rate <- build_df$value[build_df$name=="total_jobs"]/build_df$value[build_df$name=="total_workers"]
}else{
  rate <- base_df$value[base_df$name=="total_jobs"]/base_df$value[base_df$name=="total_workers"]
}
...
# Use JPEGs for ABM vs ABM run
if(SCATTER_JPEG=="Yes"){
  
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Jobs_Workers_Management.PNG"))
  
}else{
  
  counts_df <- build_data[[which(build_csv_names=="job_worker_Summary")]]
  counts_df <- counts_df[counts_df$occp=="Management",]
  ...
  colnames(counts_df) <- c("CountLocation", "OCCTYPE", "y", "x")
  
  #remove rows where both x and y are zeros
  counts_df <- counts_df[!(counts_df$x==0 & counts_df$y==0),]
  lab_pos_x <- 0.25*max(counts_df$x)
  lab_pos_y <- 0.75*max(counts_df$y)
  lab_45l_x <- 0.75*max(counts_df$x)
  ...
  p2 <- ggplot(counts_df, aes(x=x, y=y)) + 
    ...
    geom_text(x = lab_pos_x, y = lab_pos_y,label = lm_eqn_intercept(counts_df),  parse = TRUE, color = "#0072B2", size = 6) + 
    geom_text(x = lab_45l_x, y = 0,label = "- - - - : 45 Deg Line",  parse = TRUE, color = "black") + 
...
### White Collar{data-height=575}
```{r job_wrk2}

# Use PNGs for ABM vs ABM run
if(SCATTER_JPEG=="Yes"){
  
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Jobs_Workers_White Collar.PNG"))
  
}else{
  
  counts_df <- build_data[[which(build_csv_names=="job_worker_Summary")]]
  counts_df <- counts_df[counts_df$occp=="White Collar",]
...
### Blue Collar{data-height=575}
```{r job_wrk3}

# Use JPEGs for ABM vs ABM run
if(SCATTER_JPEG=="Yes"){
  
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Jobs_Workers_Blue Collar.PNG"))
  
}else{
  
  counts_df <- build_data[[which(build_csv_names=="job_worker_Summary")]]
  counts_df <- counts_df[counts_df$occp=="Blue Collar",]
...
### Sales & marketing{data-height=575}
```{r job_wrk4}

# Use JPEGs for ABM vs ABM run
if(SCATTER_JPEG=="Yes"){
  
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Jobs_Workers_Sales & marketing.PNG"))
  
}else{
  
  counts_df <- build_data[[which(build_csv_names=="job_worker_Summary")]]
  counts_df <- counts_df[counts_df$occp=="Sales & marketing",]
...
### Natural Resources{data-height=575}
```{r job_wrk5}

# Use JPEGs for ABM vs ABM run
if(SCATTER_JPEG=="Yes"){
  
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Jobs_Workers_Natural Resources.PNG"))
  
}else{
  
  counts_df <- build_data[[which(build_csv_names=="job_worker_Summary")]]
  counts_df <- counts_df[counts_df$occp=="Natural Resources",]
...
### Production{data-height=575}
```{r job_wrk6}

# Use JPEGs for ABM vs ABM run
if(SCATTER_JPEG=="Yes"){
  
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Jobs_Workers_Production.PNG"))
  
}else{
  
  counts_df <- build_data[[which(build_csv_names=="job_worker_Summary")]]
  counts_df <- counts_df[counts_df$occp=="Production",]
...
if(SCATTER_JPEG=="Yes"){
  ...
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Jobs_Workers_Total.PNG"))
  
}else{
  
  counts_df <- build_data[[which(build_csv_names=="job_worker_Summary")]]
  counts_df <- counts_df[counts_df$occp=="Total",]
...
Enrollment vs Students{data-navmenu="Long Term"}
============================================
...
**Total enrollment vs Number of students comparison at MAZ level**

Results of school location model.

Comparison of assigned students to enrollments at MAZ level.
...
### Grade K-8{data-height=575}
```{r enrl_stud1}

# Use JPEGs for ABM vs ABM run
if(SCATTER_JPEG=="Yes"){
  
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Students_Enrollments_Grade_K_8.PNG"))
  
}else{
  
  counts_df <- build_data[[which(build_csv_names=="enrollment_students_Summary")]]
  counts_df <- counts_df[counts_df$studCat=="Grade_K_8",]
  ...
  lab_pos_x <- 0.25*max(counts_df$x)
  lab_pos_y <- 0.75*max(counts_df$y)
  lab_45l_x <- 0.80*max(counts_df$x)
  ...
  p2 <- ggplot(counts_df, aes(x=x, y=y)) + 
    ...
    geom_text(x = lab_pos_x, y = lab_pos_y,label = lm_eqn_intercept(counts_df),  parse = TRUE, color = "#0072B2", size = 6) + 
    geom_text(x = lab_45l_x, y = 0,label = "- - - - : 45 Deg Line",  parse = TRUE, color = "black") + 
    labs(x="Enrollments-Grade_K_8", y="Students-Grade_K_8")
  p2 <- plotly_build(p2)
  p2
...
### Grade 9-12{data-height=575}
```{r enrl_stud2}

# Use PNGs for ABM vs ABM run
if(SCATTER_JPEG=="Yes"){
  
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Students_Enrollments_Grade_9_12.PNG"))
  
}else{
  
  counts_df <- build_data[[which(build_csv_names=="enrollment_students_Summary")]]
  counts_df <- counts_df[counts_df$studCat=="Grade_9_12",]
  ...
  p2 <- ggplot(counts_df, aes(x=x, y=y)) + 
    ...
    geom_text(x = lab_pos_x, y = lab_pos_y,label = lm_eqn_intercept(counts_df),  parse = TRUE, color = "#0072B2", size = 6) + 
    geom_text(x = lab_45l_x, y = 0,label = "- - - - : 45 Deg Line",  parse = TRUE, color = "black") + 
    labs(x="Enrollments-Grade_9_12", y="Students-Grade_9_12")
  p2 <- plotly_build(p2)
  p2  
...
### University{data-height=575}
```{r enrl_stud3}

# Use PNGs for ABM vs ABM run
if(SCATTER_JPEG=="Yes"){
  
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Students_Enrollments_University.PNG"))
  
}else{
  
  counts_df <- build_data[[which(build_csv_names=="enrollment_students_Summary")]]
  counts_df <- counts_df[counts_df$studCat=="University",]
  ...
  p2 <- ggplot(counts_df, aes(x=x, y=y)) + 
    ...
    geom_text(x = lab_pos_x, y = lab_pos_y,label = lm_eqn_intercept(counts_df),  parse = TRUE, color = "#0072B2", size = 6) + 
    geom_text(x = lab_45l_x, y = 0,label = "- - - - : 45 Deg Line",  parse = TRUE, color = "black") + 
    labs(x="Enrollments-University", y="Students-University")
  p2 <- plotly_build(p2)
  p2  
...
if(SCATTER_JPEG=="Yes"){
  ...
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Students_Enrollments_Total.PNG"))
  
}else{
  
  counts_df <- build_data[[which(build_csv_names=="enrollment_students_Summary")]]
  counts_df <- counts_df[counts_df$studCat=="Total",]
  ...
  p2 <- ggplot(counts_df, aes(x=x, y=y)) + 
    ...
    geom_text(x = lab_pos_x, y = lab_pos_y,label = lm_eqn_intercept(counts_df),  parse = TRUE, color = "#0072B2", size = 6) + 
    geom_text(x = lab_45l_x, y = 0,label = "- - - - : 45 Deg Line",  parse = TRUE, color = "black") + 
    labs(x="Enrollments-Total", y="Students-Total")
  p2 <- plotly_build(p2)
  p2  
...
_EA_: 3:00 AM to 7:00 AM

_AM_: 7:00 AM to 8:30 AM

_MD_: 8:30 AM to 4:30 PM

_PM_: 4:30 PM to 6:30 PM

_EV_: 6:30 PM to 3:00 AM
...
```{r tourMode}
if(IS_BASE_SURVEY=="Yes"){
  base_df <- base_data[[which(base_csv_names=="tmodeProfile_vis_OBS")]]
}else{
...
p1 <- plotly_bar_plotter(data = sd.pertype2, height = 350, xlabel = "Number of Stops", ylabel = "Percent", ynames = c("SD-HTS", BUILD_SCENARIO_NAME), percent = T, tickvals = seq(1,7), ticktext = c("0", "1", "2", "3", "4", "5", "6pl"))
...
```{r tripMode}
if(IS_BASE_SURVEY=="Yes"){
  base_df <- base_data[[which(base_csv_names=="tripModeProfile_vis_OBS")]]
...
```{r count_vol1}
if(IS_BASE_SURVEY=="Yes"){
  # both count and volume summaries for count_locations are in the build summary folder
  base_df <- build_data[[which(build_csv_names=="countLocCounts")]]
  build_df <- build_data[[which(build_csv_names=="countLocVolumes")]]
}else{
  #for non-survey base, all links summaries are created
  #a unique ID needs to be created using from node and to node IDs
  base_df <- base_data[[which(base_csv_names=="allLinkSummary")]]
  base_df$id <- paste(base_df$From_Node, base_df$To_Node, sep = "-")
  build_df <- build_data[[which(build_csv_names=="allLinkSummary")]]
  build_df$id <- paste(build_df$From_Node, build_df$To_Node, sep = "-")
}

#keep only records that are common between base and build scenario
common_ids <- intersect(unique(base_df$id), unique(build_df$id))
base_df <- base_df[base_df$id %in% common_ids, ]
build_df <- build_df[build_df$id %in% common_ids, ]
base_df <- base_df[order(base_df$id), ]
build_df <- build_df[order(build_df$id), ]
...
eq <- counts_df %>% group_by(FACTYPE) %>% do(V1 = lm_eqn_intercept(.))
y_pos <- c(y_max*0.675, y_max*0.70, y_max*0.725, y_max*0.75, y_max*0.775, y_max*0.800, y_max*0.825)
eq$y_pos <- y_pos
y_lab_pos <- 0.75*y_max
x_lab_pos <- 0.2*x_max
x_45l_pos <- 0.75*x_max

p1 <- ggplot(counts_df, aes(x=x, y=y, color=FACTYPE)) + 
  geom_point(shape=1) + 
  geom_smooth(method=lm, formula = y ~ x, se=FALSE) + 
  geom_abline(intercept = 0, slope = 1, linetype = 2) + 
  geom_text(data = eq, aes(x = x_lab_pos, y = y_pos,label=V1),  parse = FALSE) + 
  geom_text(x = x_45l_pos, y = 0,label = "-----:45DegLine",  parse = FALSE, color = "black") + 
  labs(x=ifelse(IS_BASE_SURVEY=="Yes", "24 Hours Counts", paste("24 Hours ", BASE_SCENARIO_NAME, " Volume", sep = "")), 
       y=ifelse(IS_BASE_SURVEY=="Yes", "24 Hours Volume", paste("24 Hours ", BUILD_SCENARIO_NAME, " Volume", sep = "")))
ggsave(paste(SYSTEM_JPEG_PATH, "Daily_FT.PNG", sep = "\\"), plot = p1, width=12,height=10, device = "png", dpi = 200)
p1 <- plotly_build(p1)
if(SCATTER_JPEG=="No"){
  p1
}else{
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Daily_FT.PNG"))
}
...
p2 <- ggplot(counts_df, aes(x=x, y=y)) + 
  ...
  geom_smooth(method=lm, formula = y ~ x, se=FALSE, color = "#0072B2") + 
  geom_abline(intercept = 0, slope = 1, linetype = 2) + 
  geom_text(x = x_lab_pos, y = y_lab_pos,label = lm_eqn_intercept(counts_df),  parse = FALSE, color = "#0072B2", size = 6) + 
  geom_text(x = x_45l_pos, y = 0,label = "- - - - : 45 Deg Line",  parse = FALSE, color = "black") + 
  labs(x=ifelse(IS_BASE_SURVEY=="Yes", "24 Hours Counts", paste("24 Hours ", BASE_SCENARIO_NAME, " Volume", sep = "")), 
       y=ifelse(IS_BASE_SURVEY=="Yes", "24 Hours Volume", paste("24 Hours ", BUILD_SCENARIO_NAME, " Volume", sep = "")))
ggsave(paste(SYSTEM_JPEG_PATH, "Daily_All.PNG", sep = "\\"), plot = p2, width=12,height=10, device = "png", dpi = 200)
p2 <- plotly_build(p2)
...
if(SCATTER_JPEG=="No"){
  ...
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "Daily_All.PNG"))
}
...
### AM Count vs Volume by Facility Type{data-height=575}
```{r count_vol4}
if(IS_BASE_SURVEY=="Yes"){
  # both count and volume summaries for count_locations are in the build summary folder
  base_df <- build_data[[which(build_csv_names=="countLocCounts")]]
...
p1 <- ggplot(counts_df, aes(x=x, y=y, color=FACTYPE)) + 
  ...
  geom_text(data = eq, aes(x = x_lab_pos, y = y_pos,label=V1),  parse = FALSE) + 
  geom_text(x = x_45l_pos, y = 0,label = "- - - - : 45 Deg Line",  parse = FALSE, color = "black") + 
  labs(x=ifelse(IS_BASE_SURVEY=="Yes", "AM Counts", paste("AM ", BASE_SCENARIO_NAME, " Volume", sep = "")), 
       y=ifelse(IS_BASE_SURVEY=="Yes", "AM Volume", paste("AM ", BUILD_SCENARIO_NAME, " Volume", sep = "")))
ggsave(paste(SYSTEM_JPEG_PATH, "AM_FT.PNG", sep = "\\"), plot = p1, width=12,height=10, device = "png", dpi = 200)
p1 <- plotly_build(p1)
if(SCATTER_JPEG=="No"){
  p1
}else{
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "AM_FT.PNG"))
}
...
p2 <- ggplot(counts_df, aes(x=x, y=y)) + 
  ...
  labs(x=ifelse(IS_BASE_SURVEY=="Yes", "AM Counts", paste("AM ", BASE_SCENARIO_NAME, " Volume", sep = "")), 
       y=ifelse(IS_BASE_SURVEY=="Yes", "AM Volume", paste("AM ", BUILD_SCENARIO_NAME, " Volume", sep = "")))
ggsave(paste(SYSTEM_JPEG_PATH, "AM_All.PNG", sep = "\\"), plot = p2, width=12,height=10, device = "png", dpi = 200)
p2 <- plotly_build(p2)
...
if(SCATTER_JPEG=="No"){
  ...
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "AM_All.PNG"))
}
```
...
### MD Count vs Volume by Facility Type{data-height=575}
```{r count_vol7}
if(IS_BASE_SURVEY=="Yes"){
  # both count and volume summaries for count_locations are in the build summary folder
  base_df <- build_data[[which(build_csv_names=="countLocCounts")]]
...
p1 <- ggplot(counts_df, aes(x=x, y=y, color=FACTYPE)) + 
  ...
  geom_text(data = eq, aes(x = x_lab_pos, y = y_pos,label=V1),  parse = FALSE) + 
  geom_text(x = x_45l_pos, y = 0,label = "- - - - : 45 Deg Line",  parse = FALSE, color = "black") + 
  labs(x=ifelse(IS_BASE_SURVEY=="Yes", "MD Counts", paste("MD ", BASE_SCENARIO_NAME, " Volume", sep = "")), 
       y=ifelse(IS_BASE_SURVEY=="Yes", "MD Volume", paste("MD ", BUILD_SCENARIO_NAME, " Volume", sep = "")))
ggsave(paste(SYSTEM_JPEG_PATH, "MD_FT.PNG", sep = "\\"), plot = p1, width=12,height=10, device = "png", dpi = 200)
p1 <- plotly_build(p1)
if(SCATTER_JPEG=="No"){
  p1
}else{
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "MD_FT.PNG"))
}
...
p2 <- ggplot(counts_df, aes(x=x, y=y)) + 
  ...
  labs(x=ifelse(IS_BASE_SURVEY=="Yes", "MD Counts", paste("MD ", BASE_SCENARIO_NAME, " Volume", sep = "")), 
       y=ifelse(IS_BASE_SURVEY=="Yes", "MD Volume", paste("MD ", BUILD_SCENARIO_NAME, " Volume", sep = "")))
ggsave(paste(SYSTEM_JPEG_PATH, "MD_All.PNG", sep = "\\"), plot = p2, width=12,height=10, device = "png", dpi = 200)
p2 <- plotly_build(p2)
...
if(SCATTER_JPEG=="No"){
  ...
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "MD_All.PNG"))
}
```
...
### PM Count vs Volume by Facility Type{data-height=575}
```{r count_vol10}
if(IS_BASE_SURVEY=="Yes"){
  # both count and volume summaries for count_locations are in the build summary folder
  base_df <- build_data[[which(build_csv_names=="countLocCounts")]]
...
p1 <- ggplot(counts_df, aes(x=x, y=y, color=FACTYPE)) + 
  ...
  geom_text(data = eq, aes(x = x_lab_pos, y = y_pos,label=V1),  parse = FALSE) + 
  geom_text(x = x_45l_pos, y = 0,label = "- - - - : 45 Deg Line",  parse = FALSE, color = "black") + 
  labs(x=ifelse(IS_BASE_SURVEY=="Yes", "PM Counts", paste("PM ", BASE_SCENARIO_NAME, " Volume", sep = "")), 
       y=ifelse(IS_BASE_SURVEY=="Yes", "PM Volume", paste("PM ", BUILD_SCENARIO_NAME, " Volume", sep = "")))
ggsave(paste(SYSTEM_JPEG_PATH, "PM_FT.PNG", sep = "\\"), plot = p1, width=12,height=10, device = "png", dpi = 200)
p1 <- plotly_build(p1)
if(SCATTER_JPEG=="No"){
  p1
}else{
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "PM_FT.PNG"))
}
...
p2 <- ggplot(counts_df, aes(x=x, y=y)) + 
  ...
  labs(x=ifelse(IS_BASE_SURVEY=="Yes", "PM Counts", paste("PM ", BASE_SCENARIO_NAME, " Volume", sep = "")), 
       y=ifelse(IS_BASE_SURVEY=="Yes", "PM Volume", paste("PM ", BUILD_SCENARIO_NAME, " Volume", sep = "")))
ggsave(paste(SYSTEM_JPEG_PATH, "PM_All.PNG", sep = "\\"), plot = p2, width=12,height=10, device = "png", dpi = 200)
p2 <- plotly_build(p2)
...
if(SCATTER_JPEG=="No"){
  ...
  knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "PM_All.PNG"))
}
```
...
```{r Table1_VMT}
if(IS_BASE_SURVEY!="Yes"){
  cat("Vehicle Miles Traveled: ", BASE_SCENARIO_NAME)

  base_df <- base_data[[which(base_csv_names=="vmtSummary")]]
  df <- base_df
  colnames(df) <- c("TOD", "SOV","HOV2","HOV3", "Truck", "Total")
  df_per <- df
  df_per[,sapply(df,is.numeric)] <- df_per[,sapply(df,is.numeric)]/df_per[,c("Total")]*100
...
```{r Table2_VMT}
if(IS_BASE_SURVEY!="Yes"){
  cat("Vehicle Miles Traveled: ", BASE_SCENARIO_NAME, " - Percentages")

  eval_expr <- paste("t2 <- kable(df_per, format = 'html', digits = 1, row.names = F, align = 'r', format.args = list(big.mark = ',')) %>%
    kable_styling('striped', font_size = 12, full_width=F, position='center') %>%
    add_header_above(c(' ' = 1, '", BASE_SCENARIO_NAME, "' = 5))", sep = "")
  eval(parse(text = eval_expr))
  t2
...
### {data-height=280}
```{r Table3_VMT}
cat("Vehicle Miles Traveled: ", BUILD_SCENARIO_NAME)
...
eval_expr <- paste("t3 <- kable(df, format = 'html', digits = 0, row.names = F, align = 'r', format.args = list(big.mark = ',')) %>%
  kable_styling('striped', font_size = 12, full_width=F, position='center') %>%
...
### {data-height=280}
```{r Table4_VMT}
cat("Vehicle Miles Traveled: ", BUILD_SCENARIO_NAME, " - Percentages")

eval_expr <- paste("t4 <- kable(df_per, format = 'html', digits = 1, row.names = F, align = 'r', format.args = list(big.mark = ',')) %>%
  kable_styling('striped', font_size = 12, full_width=F, position='center') %>%
  add_header_above(c(' ' = 1, '", BUILD_SCENARIO_NAME, "' = 5))", sep = "")
eval(parse(text = eval_expr))
t4
...
Transit Boardings{data-navmenu="Assignment"}
============================================
...
Daily transit boardings by route
...
cat("Transit Boardings:", BOARDINGS_SOURCE)
base_df <- base_data[[which(base_csv_names=="transitBoardingSummary")]]
build_df <- build_data[[which(build_csv_names=="transitBoardingSummary")]]
union_routes <- union(colnames(base_df), colnames(build_df))
df <- base_df[,union_routes]
df[is.na(df)] <- 0

eval_expr <- paste("t1 <- kable(df, format = 'html', digits = 0, row.names = F, align = 'c', format.args = list(big.mark = ',')) %>%
  kable_styling('striped', font_size = 10, full_width=F, position='center') %>%
  add_header_above(c(' ' = 1, '", BASE_SCENARIO_NAME, "' = ", ncol(df)-1,"))", sep = "")
eval(parse(text = eval_expr))
t1
...
### 
```{r Table2_transit}
cat("Transit Boardings:", BUILD_SCENARIO_NAME)

build_df <- build_data[[which(build_csv_names=="transitBoardingSummary")]]
df <- build_df[,union_routes]
df[is.na(df)] <- 0

eval_expr <- paste("t1 <- kable(df, format = 'html', digits = 0, row.names = F, align = 'c', format.args = list(big.mark = ',')) %>%
  kable_styling('striped', font_size = 10, full_width=F, position='center') %>%
  add_header_above(c(' ' = 1, '", BUILD_SCENARIO_NAME, "' = ", ncol(df)-1, "))", sep = "")
eval(parse(text = eval_expr))
t1
```
...
CVM Trips{data-navmenu="Aggregate"}
============================================
...
```{r Table1_cvm}
if(IS_BASE_SURVEY!="Yes"){
  cat("CVM Trips:", BASE_SCENARIO_NAME)
...
### {data-height=280}
```{r Table2_cvm}
cat("CVM Trips:", BUILD_SCENARIO_NAME)
...
CVM VMT{data-navmenu="Aggregate"}
============================================
...
Total VMT by time of day and mode, computed using SOV-AM distance skim
...
```{r Table3_cvm}
if(IS_BASE_SURVEY!="Yes"){
  cat("CVM VMT:", BASE_SCENARIO_NAME)

  base_df <- base_data[[which(base_csv_names=="cvm_vmt_summary")]]
  df <- base_df
  colnames(df) <- c("TOD", "Car","Multi-Unit Truck","Single-Unit Truck", "Total")
  df$TOD <- factor(df$TOD, levels = aggTimePeriods)
  df <- df[order(df$TOD),]
...
### {data-height=280}
```{r Table4_cvm}
cat("CVM VMT:", BUILD_SCENARIO_NAME)

build_df <- build_data[[which(build_csv_names=="cvm_vmt_summary")]]
df <- build_df
colnames(df) <- c("TOD", "Car","Multi-Unit Truck","Single-Unit Truck", "Total")
df$TOD <- factor(df$TOD, levels = aggTimePeriods)
df <- df[order(df$TOD),]
...
External Trips{data-navmenu="Aggregate"}
============================================
...
```{r Table1_ext}
if(IS_BASE_SURVEY!="Yes"){
  cat("External Trips:", BASE_SCENARIO_NAME)
...
### {data-height=280}
```{r Table2_ext}
cat("External Trips:", BUILD_SCENARIO_NAME)
...
External VMT{data-navmenu="Aggregate"}
============================================
...
Total VMT by time of day and purpose, computed using SOV-AM distance skim
...
```{r Table3_ext}
if(IS_BASE_SURVEY!="Yes"){
  cat("External VMT:", BASE_SCENARIO_NAME)

  base_df <- base_data[[which(base_csv_names=="ext_vmt_summary")]]
  df <- base_df
  colnames(df) <- c("TOD", "HBCOLL","HBO","HBR", "HBS", "HBSCH", "HBW", "NHBNW", "NHBW", "Truck", "Total")
  df$TOD <- factor(df$TOD, levels = aggTimePeriods)
  df <- df[order(df$TOD),]
...
### {data-height=280}
```{r Table4_ext}
cat("External VMT:", BUILD_SCENARIO_NAME)

build_df <- build_data[[which(build_csv_names=="ext_vmt_summary")]]
df <- build_df
colnames(df) <- c("TOD", "HBCOLL","HBO","HBR", "HBS", "HBSCH", "HBW", "NHBNW", "NHBW", "Truck", "Total")
df$TOD <- factor(df$TOD, levels = aggTimePeriods)
df <- df[order(df$TOD),]
