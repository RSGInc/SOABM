...
### Base Jobs
```{r Jobs1_ValueBox}
# jobs when base is survey, else number of workers
if(IS_BASE_SURVEY=="Yes"){
  ...
  valueBox(prettyNum(round(base_df$value[base_df$name=="total_workers"]/BASE_SAMPLE_RATE), big.mark = ","), "Workers", icon = "fas fa-industry")
}
...
### Average HH Size
```{r AvgHHSize4_Gauge}
rate <- build_df$value[build_df$name=="total_population"]/build_df$value[build_df$name=="total_households"]
gauge(prettyNum(round(rate, 2), big.mark = ","), min = 0, max = 5, gaugeSectors(danger = c(0,5), colors = c("Green", "Green", "Green")))
```
...
cat("Transit Boardings:", BASE_SCENARIO_NAME)
...
common_routes <- intersect(colnames(base_df), colnames(build_df))
df <- base_df[,common_routes]
