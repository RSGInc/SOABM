...
cvm_tod <- c("EV1","EA","AM","MD","PM","EV2",
             "EV1","EA","AM","MD","PM","EV2",
             "EV1","EA","AM","MD","PM","EV2")
cvm_mode <- c("car","car","car","car","car","car",
              "su" ,"su" ,"su" ,"su" ,"su" ,"su" ,
              "mu" ,"mu" ,"mu" ,"mu" ,"mu" ,"mu" )
cvm_trips <- c()
...
rw <- data.frame("Daily", sum(cvm_df$car), sum(cvm_df$mu), sum(cvm_df$su))
colnames(rw) <- colnames(cvm_df)
...
ext_mtx <- c("EV1_hbw",    "EA_hbw",    "AM_hbw",    "MD_hbw",    "PM_hbw",    "EV2_hbw", 
             ...
             "EV1_hbo",    "EA_hbo",    "AM_hbo",    "MD_hbo",    "PM_hbo",    "EV2_hbo",
             "EV1_hbr",    "EA_hbr",    "AM_hbr",    "MD_hbr",    "PM_hbr",    "EV2_hbr",
             "EV1_hbs",    "EA_hbs",    "AM_hbs",    "MD_hbs",    "PM_hbs",    "EV2_hbs",
             "EV1_nhbnw",  "EA_nhbnw",  "AM_nhbnw",  "MD_nhbnw",  "PM_nhbnw",  "EV2_nhbnw",
             "EV1_nhbw",   "EA_nhbw",   "AM_nhbw",   "MD_nhbw",   "PM_nhbw",   "EV2_nhbw",
             "EV1_truck",  "EA_truck",  "AM_truck",  "MD_truck",  "PM_truck",  "EV2_truck")
ext_tod <- c("EV1","EA","AM","MD","PM","EV2",
             "EV1","EA","AM","MD","PM","EV2",
             "EV1","EA","AM","MD","PM","EV2",
             "EV1","EA","AM","MD","PM","EV2",
             "EV1","EA","AM","MD","PM","EV2",
             ...
             "EV1","EA","AM","MD","PM","EV2",
             "EV1","EA","AM","MD","PM","EV2")
ext_purp <- c("hbw",    "hbw",    "hbw",    "hbw",    "hbw",    "hbw", 
              "hbcoll", "hbcoll", "hbcoll", "hbcoll", "hbcoll", "hbcoll",
              "hbsch",  "hbsch",  "hbsch",  "hbsch",  "hbsch",  "hbsch",
              "hbo",    "hbo",    "hbo",    "hbo",    "hbo",    "hbo",
              "hbr",    "hbr",    "hbr",    "hbr",    "hbr",    "hbr",
              "hbs",    "hbs",    "hbs",    "hbs",    "hbs",    "hbs",
              "nhbnw",  "nhbnw",  "nhbnw",  "nhbnw",  "nhbnw",  "nhbnw",
...
rw <- data.frame("Daily", sum(ext_df$hbcoll), sum(ext_df$hbo), sum(ext_df$hbr), sum(ext_df$hbs), sum(ext_df$hbsch), 
                 sum(ext_df$hbw), sum(ext_df$nhbnw), sum(ext_df$nhbw), sum(ext_df$truck))
colnames(rw) <- colnames(ext_df)
...
# Districts are DISTNAME
wsLoc$HDISTRICT <- xwalk$DISTNAME[match(wsLoc$HomeMGRA, xwalk$MAZ)]
wsLoc$WDISTRICT <- xwalk$DISTNAME[match(wsLoc$WorkLocation, xwalk$MAZ)]
...
hh$HHTAZ <- xwalk$TAZ[match(hh$home_maz, xwalk$NO)]
...
# code distance bins
workers$distbin <- cut(workers$WorkLocationDistance, breaks = c(seq(0,40, by=1), 9999), labels = F, right = F)
students$distbin <- cut(students$SchoolLocationDistance, breaks = c(seq(0,40, by=1), 9999), labels = F, right = F)

distBinCat <- data.frame(distbin = seq(1,41, by=1))
...
tours$OTAZ <- xwalk$TAZ[match(tours$orig_maz, xwalk$MAZ)]
tours$DTAZ <- xwalk$TAZ[match(tours$dest_maz, xwalk$MAZ)]
...
unique_joint_tours$OTAZ <- xwalk$TAZ[match(unique_joint_tours$orig_maz, xwalk$MAZ)]
unique_joint_tours$DTAZ <- xwalk$TAZ[match(unique_joint_tours$dest_maz, xwalk$MAZ)]
...
suppressWarnings(write.table(jtfSummary, "jtfSummary.csv", col.names=TRUE, sep=",")) 
suppressWarnings(write.table(jointComp, "jtfSummary.csv", col.names=TRUE, sep=",", append=TRUE)) 
suppressWarnings(write.table(jointPartySize, "jtfSummary.csv", col.names=TRUE, sep=",", append=TRUE)) 
suppressWarnings(write.table(jointCompPartySize, "jtfSummary.csv", col.names=TRUE, sep=",", append=TRUE)) 
suppressWarnings(write.table(jointToursHHSize, "jtfSummary.csv", col.names=TRUE, sep=",", append=TRUE)) 
